...
for (const tz of timezones) {
  minuteOffsets.push({
    ...
    o: tz.currentTimeOffsetInMinutes,
...
function setClientOffset() {
  ...
  fetch(`https://time.filiptronicek.workers.dev/?ts=${timestamp}`)
    .then((f) => f.json())
    .then((f) => {
      const nowstamp = Date.now();
      const adjustedOffset = Math.round(
        f.result.ms - (nowstamp - timestamp) / 2
      );
...
if (
  ...
  (localStorage.getItem("offsetUpdate") &&
    Math.abs(parseInt(localStorage.getItem("offsetUpdate") - Date.now())) >
      1000 * 60 * 60 * 24)
) {
  setClientOffset();
}
...
const toCountDownDate =
  parseInt(urlParams.get("d")) || urlParams.get("d") || desiredDFLTtime;
const countDownDate = new Date(toCountDownDate).getTime();
const countDownName = urlParams.get("n") || desiredDFLTname;

const confettiEf = urlParams.has("confetti")
...
const x = setInterval(() => {
  ...
  const now = new Date().getTime() + parseInt(localStorage.getItem("offset"));
  ...
  if (distance < 86400 * 1000 && output.innerHTML === "") {
    ...
    for (const offset of reducedOffsets) {
      ...
      const desiredDate = dayjs(countDownDate).add(
        dayjs(countDownDate).utcOffset(),
        "minute"
      );

      if (
        offsetedDate.format("MM/DD") === desiredDate.format("MM/DD") &&
        offsetedDate.format("hh:mm:ss") === desiredDate.format("hh:mm:ss")
      ) {
        timezoneText.innerHTML =
          "You just hit your countdown in these timezones:";
  ...
  const correctedToWithoutMonthDistance = Math.floor(
    (daywithoutMdiff - now) / (1000 * 60 * 60 * 24)
  );

  const totaldays = Math.floor(distance / (1000 * 60 * 60 * 24));
  const days = correctedToWithoutMonthDistance % 7;
  const hours = Math.floor(
    ((daywithoutMdiff - now) % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
  );
  const minutes = Math.floor(
    ((daywithoutMdiff - now) % (1000 * 60 * 60)) / (1000 * 60)
  );
  const seconds = Math.floor(((daywithoutMdiff - now) % (1000 * 60)) / 1000);

  const weeks =
    (correctedToWithoutMonthDistance - (correctedToWithoutMonthDistance % 7)) /
    7;
      ...
      if (days > 0) {
        ...
        if (hours > 0) {
          ...
          if (minutes > 0) {
            ...
            writeOnOut(
              seconds + "," + Math.floor((distance % 1000) / 100) + " seconds "
            );
          } else {
